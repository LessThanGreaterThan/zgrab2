...
func RegisterModule() {
	var m Module
	_, err := zgrab2.AddCommand("moobot", "moobot", m.Description(), 42061, &m)
	if err != nil {
		log.Fatal(err)
	}
}
...
func (s *Scanner) Scan(target zgrab2.ScanTarget) (zgrab2.ScanStatus, interface{}, error) {
	...
	defer conn.Close()
	_, err = conn.Write([]byte{0x33, 0x66, 0x99, 0x01, 0x63})
	if err != nil {
		return zgrab2.SCAN_CONNECTION_CLOSED, nil, err
	}

	bannerSlice, err := zgrab2.ReadAvailableWithOptions(conn, 3, 500*time.Millisecond, 0, 3)
	if bannerSlice != nil {
		if bytes.Equal(bannerSlice, []byte{0x33, 0x66, 0x99}) {
